...
const chain = defineChain({
  id: CHAIN_ID,
  name: "Taiko Mainnet (fork)",
  nativeCurrency: { name: "Ether", symbol: "ETH", decimals: 18 },
  rpcUrls: { default: { http: [RPC_URL] } },
});

async function deriveX25519FromWallet(
  privateKey: Hex,
  keyNonce: number,
) {
...
async function main() {
  ...
  try {
    ...
    log("verifying deployed contract is accessible...");
    const maxFee = await publicClient.readContract({
      address: CONTRACT,
      abi: [{ type: "function", name: "MAX_FEE_GWEI", inputs: [],
        outputs: [{ type: "uint32" }], stateMutability: "view" }],
      functionName: "MAX_FEE_GWEI",
    });
    assert.equal(maxFee, 10_000_000, "MAX_FEE_GWEI matches deployment");
    log(`deployed Heed at ${CONTRACT}, MAX_FEE_GWEI = ${maxFee}`);
    ...
    log("publishing encryption keys + setting bob's fee to 100 gwei...");
    ...
    const unsigned: UnsignedEnvelope = {
      ...
      title: "deploy succeeded (mainnet fork)",
      body: "build #1234 is live on Taiko mainnet",
      urgency: "normal",
    ...
    const inbox = await pollUntil(async () => {
      const items = await source.listInbox(
        bob.address,
        DEPLOYED_AT_BLOCK,
        50,
      );
    ...
    assert.equal(
      msg.contentRef.toLowerCase(),
      contentRef.toLowerCase(),
      "contentRef matches",
    );
    ...
    assert.equal(
      env.from.owner_url,
      signed.from.owner_url,
      "owner_url round-trips",
    );
    ...
    process.stderr.write(
      `e2e-fork: FAIL — ${(err as Error).stack ?? (err as Error).message}\n`,
    );
    exitCode = 1;
  } finally {
    await stub.close();
...
function log(msg: string): void {
  process.stderr.write(`[e2e-fork] ${msg}\n`);
}
