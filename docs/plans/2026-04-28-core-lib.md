# `postage-core` Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship a TypeScript library that handles every piece of Postage's protocol surface (key derivation, lockbox encryption, payload codecs, contract bindings, IPFS, mail sources) so any client — web, native, agent — can integrate without re-implementing crypto.
...
**Spec source:** `docs/postage-design.md`. Depends on Plan 1 ABI artifact (`contracts/out/Postage.sol/Postage.json`).
...
```bash
git add core/
git commit -m "Bootstrap postage-core package (TS + viem + noble + vitest)"
```
...
const INFO = new TextEncoder().encode("postage.lockbox.v1");
...
node -e "const j = require('../contracts/out/Postage.sol/Postage.json'); console.log('export const POSTAGE_ABI =', JSON.stringify(j.abi, null, 2), 'as const;')" > core/src/contract/abi.ts
```
...
describe("read client (anvil)", () => {
  ...
  beforeAll(async () => {
    ...
    address = process.env.POSTAGE_ADDRESS as `0x${string}`;
  });
...
```bash
git commit -am "Add viem-based read client for Postage contract"
```
...
export function createWriteClient(wallet: WalletClient, address: `0x${string}`) {
  const write = (functionName: string, args: any[], value = 0n) =>
    wallet.writeContract({ address, abi: POSTAGE_ABI, functionName, args, value, chain: wallet.chain, account: wallet.account! });
...
```bash
git commit -am "Release postage-core v0.1.0"
```
