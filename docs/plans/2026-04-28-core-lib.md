...
**Tasks 1–12 complete; Task 13 Steps 1–2 (build + package metadata) done.** 37 vitest tests pass, `tsc --noEmit` clean, `tsup` produces ESM + `.d.ts`.

Three follow-ups from PR #5 review fixed in commit `2d7dc5e`: IndexerMailSource forwards `sinceBlock` as `blockNumber_gte`; RpcMailSource fetches real `blockTimestamp` via deduped `getBlock`; both backends return descending order.

**Plan 1 dependency:** [`Heed`](https://taikoscan.io/address/0x08f32278B2CFD962444ae9541122eD84cc745678) is live on Taiko mainnet at `0x08f32278B2CFD962444ae9541122eD84cc745678` (block 6091023). Verified ABI matches `core-lib/src/contract/abi.ts`. The library can now be exercised against mainnet directly via `createReadClient`/`createRpcMailSource`.

**Remaining:**
- Task 13 Step 3: `npm publish --access public`. The audit-gate condition is moot (audit was skipped). Publish is now an explicit owner decision — defer until ready for external consumers, or unblock [Plan 4 `heed-web`](./2026-04-28-web.md) by consuming `@heed/core` via a workspace/`file:` install in the meantime.
