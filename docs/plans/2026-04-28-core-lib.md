# `taikomail-core` Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship a TypeScript library that handles every piece of Taiko Mail's protocol surface (key derivation, lockbox encryption, payload codecs, contract bindings, IPFS, mail sources) so any client — web, native, agent — can integrate without re-implementing crypto.
...
**Spec source:** `docs/superpowers/specs/2026-04-28-taiko-mail-design.md`. Depends on Plan 1 ABI artifact (`contracts/out/TaikoMail.sol/TaikoMail.json`).
...
- Create: `core/src/index.ts` — public exports
- Create: `core/src/types.ts`
- Create: `core/src/crypto/key-derivation.ts`
- Create: `core/src/crypto/lockbox.ts`
- Create: `core/src/payload/cid.ts`
- Create: `core/src/payload/plaintext.ts`
- Create: `core/src/payload/encrypted.ts`
- Create: `core/src/contract/abi.ts`
- Create: `core/src/contract/client.ts`
- Create: `core/src/ipfs/pinata.ts`
- Create: `core/src/ipfs/gateway.ts`
- Create: `core/src/source/types.ts`
- Create: `core/src/source/rpc.ts`
- Create: `core/src/source/indexer.ts`
- Create: `core/test/**/*.test.ts`
...
```bash
git add core/
git commit -m "Bootstrap taikomail-core package (TS + viem + noble + vitest)"
```
...
**Files:**
- Create: `core/src/crypto/key-derivation.ts`
- Create: `core/test/crypto/key-derivation.test.ts`
...
**Files:**
- Create: `core/src/crypto/lockbox.ts`
- Create: `core/test/crypto/lockbox.test.ts`
...
const INFO = new TextEncoder().encode("taikomail.lockbox.v1");
...
**Files:**
- Create: `core/src/payload/cid.ts`
- Create: `core/test/payload/cid.test.ts`
...
- Create: `core/src/payload/plaintext.ts`
- Create: `core/src/payload/encrypted.ts`
- Create: `core/test/payload/codecs.test.ts`
...
- Create: `core/src/contract/abi.ts`
- Create: `core/src/contract/client.ts`
- Create: `core/test/contract/client.test.ts`

- [ ] **Step 1: Generate ABI from Foundry artifacts**

```bash
forge build --root ../contracts
node -e "const j = require('../contracts/out/TaikoMail.sol/TaikoMail.json'); console.log('export const TAIKO_MAIL_ABI =', JSON.stringify(j.abi, null, 2), 'as const;')" > core/src/contract/abi.ts
```

- [ ] **Step 2: Implement read client**

```ts
import { type Address, type PublicClient, getContract } from "viem";
import { TAIKO_MAIL_ABI } from "./abi";
import type { InboxView } from "../types";

export function createReadClient(client: PublicClient, address: Address) {
  const contract = getContract({ address, abi: TAIKO_MAIL_ABI, client });
  return {
...
```bash
git commit -am "Add viem-based read client for TaikoMail contract"
```
...
**Files:**
- Modify: `core/src/contract/client.ts`
- Modify: `core/test/contract/client.test.ts`

- [ ] **Step 1: Extend client with write methods**
...
export function createWriteClient(wallet: WalletClient, address: `0x${string}`) {
  const write = (functionName: string, args: any[], value = 0n) =>
    wallet.writeContract({ address, abi: TAIKO_MAIL_ABI, functionName, args, value, chain: wallet.chain, account: wallet.account! });
  ...
  return {
    ...
    registerDelegate: (delegate: `0x${string}`, clientId: Hex, value: bigint) =>
      write("registerDelegate", [delegate, clientId], value),
...
- [ ] **Step 2: Anvil integration test (delegate path)**
...
- Create: `core/src/ipfs/pinata.ts`
- Create: `core/src/ipfs/gateway.ts`
- Create: `core/test/ipfs/pinata.test.ts`
- Create: `core/test/ipfs/gateway.test.ts`

- [ ] **Step 1: Implement pinata.ts**
...
- [ ] **Step 3: Mock-based unit tests** (use `vi.fn()` to stub `fetch` for both error and success cases)
...
- Create: `core/src/source/types.ts`
- Create: `core/src/source/rpc.ts`
- Create: `core/test/source/rpc.test.ts`
...
- [ ] **Step 2: Implement RpcMailSource**
...
**Files:**
- Create: `core/src/source/indexer.ts`
- Create: `core/test/source/indexer.test.ts`

- [ ] **Step 1: GraphQL client**
...
- [ ] **Step 2: Mocked test** (stub `fetch` to return shaped GraphQL responses)
...
**Files:**
- Create: `core/test/e2e/round-trip.test.ts`
...
```bash
git commit -am "Release taikomail-core v0.1.0"
```
