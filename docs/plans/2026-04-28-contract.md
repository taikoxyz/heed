# TaikoMail.sol Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship the immutable `TaikoMail.sol` contract on Taiko mainnet — fully tested, audited, and verified on Taikoscan.
...
**Spec source:** `docs/superpowers/specs/2026-04-28-taiko-mail-design.md`
...
- Create: `contracts/src/interfaces/ITaikoMail.sol`
- Create: `contracts/src/TaikoMail.sol`
- Create: `contracts/test/TaikoMail.t.sol`
- Create: `contracts/test/TaikoMail.invariant.t.sol`
...
- [ ] **Step 5: Sanity-check forge build**
...
```bash
git add contracts/
git commit -m "Bootstrap Foundry workspace for TaikoMail contract"
```

---

### Task 2: Define `ITaikoMail` interface (types + events + signatures)
...
```bash
git add contracts/src/interfaces/ITaikoMail.sol
git commit -m "Add ITaikoMail interface (events, structs, errors)"
```
...
- [ ] **Step 1: Write failing test for key publish + getKeys**
...
import {TaikoMail} from "src/TaikoMail.sol";
import {ITaikoMail} from "src/interfaces/ITaikoMail.sol";

contract TaikoMailTest is Test {
    TaikoMail tm;
    address alice = makeAddr("alice");

    function setUp() public {
        tm = new TaikoMail(10_000_000);
    }
    ...
    function test_publishKey_storesAndEmits() public {
        ...
        emit ITaikoMail.KeyPublished(alice, 0, pub0);
        vm.prank(alice);
        tm.publishKey(0, pub0);

        ITaikoMail.EncKey[2] memory keys = tm.getKeys(alice);
...
Expected: FAIL — `TaikoMail` not found.

- [ ] **Step 3: Implement skeleton + publishKey + getKeys**

```solidity
// SPDX-License-Identifier: MIT
pragma solidity 0.8.27;

import {ITaikoMail} from "./interfaces/ITaikoMail.sol";

contract TaikoMail is ITaikoMail {
...
- [ ] **Step 5: Add slot rotation tests**

Append to `TaikoMail.t.sol`:
...
function test_publishKey_rejectsNonMonotonic() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(ITaikoMail.KeyNonceNotMonotonic.selector, 3, 5));
...
Run: `forge test --match-contract TaikoMailTest -vv`
Expected: 3 PASS.

- [ ] **Step 7: Commit**

```bash
git add contracts/src/TaikoMail.sol contracts/test/TaikoMail.t.sol
...
**Files:**
- Modify: `contracts/src/TaikoMail.sol`
- Modify: `contracts/test/TaikoMail.t.sol`
...
function test_setFee_storesAndEmits() public {
    vm.expectEmit(true, false, false, true, address(tm));
    emit ITaikoMail.FeeUpdated(alice, 12345);
    vm.prank(alice);
    tm.setFee(12345);
...
function test_setFee_revertsAboveCap() public {
    vm.prank(alice);
    vm.expectRevert(abi.encodeWithSelector(ITaikoMail.FeeAboveCap.selector, 10_000_001, 10_000_000));
...
- [ ] **Step 3: Implement setFee, replace stub**
...
function test_trust_addsAndEmits() public {
    ...
    emit ITaikoMail.Trusted(alice, bob, true);
    vm.expectEmit(true, true, false, true, address(tm));
    emit ITaikoMail.Trusted(alice, carol, true);
    vm.prank(alice);
    tm.trust(list);
...
function test_untrust_removesAndEmits() public {
    ...
    tm.trust(list);
    vm.expectEmit(true, true, false, true, address(tm));
    emit ITaikoMail.Trusted(alice, bob, false);
    tm.untrust(list);
    vm.stopPrank();
...
function test_registerDelegate_setsMappingsAndForwardsValue() public {
    ...
    bytes32 clientId = keccak256("taikomail-mac-v1");
    vm.deal(alice, 1 ether);

    vm.expectEmit(true, true, false, true, address(tm));
    emit ITaikoMail.DelegateRegistered(alice, delegate, clientId);
    vm.prank(alice);
...
function test_revokeDelegate_byOwner() public {
    ...
    vm.expectEmit(true, true, false, true, address(tm));
    emit ITaikoMail.DelegateRevoked(alice, delegate);
...
- [ ] **Step 2: Failing tests for atomic send**
...
function test_sendBatch_atomic_singleSuccess() public {
    ...
    ITaikoMail.MailIntent[] memory mails = new ITaikoMail.MailIntent[](1);
    mails[0] = ITaikoMail.MailIntent({
        recipient: bob,
        valueGwei: 100,
    ...
    vm.expectEmit(true, true, false, true, address(tm));
    emit ITaikoMail.MailSent(alice, bob, bytes32(uint256(0xabcd)), 100);
    vm.prank(alice);
...
function test_sendBatch_atomic_revertsOnInsufficientValue() public {
    ...
    ITaikoMail.MailIntent[] memory mails = new ITaikoMail.MailIntent[](1);
    mails[0] = ITaikoMail.MailIntent({recipient: bob, valueGwei: 50, contentRef: bytes32(0)});
...
function test_sendBatch_delegateAttribution() public {
    ...
    ITaikoMail.MailIntent[] memory mails = new ITaikoMail.MailIntent[](1);
    mails[0] = ITaikoMail.MailIntent({recipient: bob, valueGwei: 0, contentRef: bytes32(uint256(1))});

    vm.expectEmit(true, true, false, true, address(tm));
    emit ITaikoMail.MailSent(alice, bob, bytes32(uint256(1)), 0); // sender == owner, not delegate
    vm.prank(delegate);
...
function test_sendBatch_bestEffort_skipsAndRefunds() public {
    ...
    ITaikoMail.MailIntent[] memory mails = new ITaikoMail.MailIntent[](3);
    mails[0] = ITaikoMail.MailIntent({recipient: bob, valueGwei: 100, contentRef: bytes32(uint256(1))});
    mails[1] = ITaikoMail.MailIntent({recipient: address(reverter), valueGwei: 0, contentRef: bytes32(uint256(2))});
    mails[2] = ITaikoMail.MailIntent({recipient: carol, valueGwei: 50, contentRef: bytes32(uint256(3))});
...
function test_sendBatch_atomic_revertsOnReverter() public {
    Reverter reverter = new Reverter();
    ITaikoMail.MailIntent[] memory mails = new ITaikoMail.MailIntent[](1);
    mails[0] = ITaikoMail.MailIntent({recipient: address(reverter), valueGwei: 100, contentRef: bytes32(0)});
    vm.deal(alice, 1 ether);
    vm.prank(alice);
    vm.expectRevert(abi.encodeWithSelector(ITaikoMail.MailFailed.selector, 0));
    tm.sendBatch{value: 100 gwei}(mails, true);
}
```

- [ ] **Step 2: Run, verify pass** (already implemented in Task 7)
...
function test_getInbox_returnsFeeAndKeys() public {
    ...
    ITaikoMail.InboxView memory v = tm.getInbox(alice);
...
function test_getInboxes_batched() public {
    ...
    ITaikoMail.InboxView[] memory vs = tm.getInboxes(list);
...
contract Handler is Test {
    TaikoMail public tm;
    constructor(TaikoMail _tm) { tm = _tm; }
    ...
    function send(uint8 nMails, uint96 totalValue, bool atomic, address[3] memory rcpts) external {
        ...
        ITaikoMail.MailIntent[] memory mails = new ITaikoMail.MailIntent[](nMails);
        for (uint256 i; i < nMails; ++i) {
            mails[i] = ITaikoMail.MailIntent({
                recipient: rcpts[i],
...
contract TaikoMailInvariantTest is Test {
    TaikoMail tm;
    Handler handler;
...
Run: `forge test --match-contract TaikoMailInvariantTest -vv`
Expected: PASS — contract never accumulates ETH because it forwards/refunds in the same call.

- [ ] **Step 3: Commit**

```bash
git commit -am "Add invariant: TaikoMail contract holds no ETH after any sendBatch"
```
...
Expected: lines ≥ 95%, branches = 100% on `TaikoMail.sol`. Iterate tests until thresholds hit.

- [ ] **Step 2: Take gas snapshot**

Run: `forge snapshot`
Commit `.gas-snapshot`.

- [ ] **Step 3: Configure slither**
...
Run: `slither contracts/src/TaikoMail.sol --config contracts/slither.config.json`
...
contract Deploy is Script {
    function run() external returns (TaikoMail tm) {
        vm.startBroadcast();
        tm = new TaikoMail(10_000_000);
        vm.stopBroadcast();
    }
...
- [ ] **Step 2: Dry-run on Anvil mainnet fork**
...
git commit -m "Record TaikoMail mainnet deployment"
```
