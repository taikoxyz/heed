...
**Live on mainnet.** Tasks 1–10 and Task 12 (deploy + verification) shipped; Task 11 partial. Audit gate explicitly skipped per owner decision — flagged as accepted risk.

| | |
|-|-|
| `Heed` | [`0x08f32278B2CFD962444ae9541122eD84cc745678`](https://taikoscan.io/address/0x08f32278B2CFD962444ae9541122eD84cc745678) (chain 167000, block 6091023) |
| Deploy tx | `0x2e9d3f6971e1e84704a7e52b3c9ed5031dc271ee39c774f0a8a6aef946e4927c` |
| Deployer | `0x327fa3369B1D1D42120d84bc407e5865ECa7c458` |
| Constructor | `MAX_FEE_GWEI = 10_000_000` (0.01 ETH cap) |
| Compiler | solc 0.8.27 / shanghai EVM / optimizer 1M / via_ir |
| Verified | Blockscout + Taikoscan (Etherscan V2 multichain `chainid=167000`) |
| Artifacts | [`deployments/mainnet.json`](../../deployments/mainnet.json), [`contracts/README.md`](../../contracts/README.md), [`DEPLOYED.md`](../../DEPLOYED.md) |
| Tests | 30 passing: 27 unit (`HeedTest`) + 2 accounting fuzz + 1 invariant |

**Layout note:** implementation lives at `contracts/impl/Heed.sol` and the interface at `contracts/iface/IHeed.sol` (plan said `contracts/src/...`); `foundry.toml` has `src = "impl"` and a `[profile.layer2]` profile with `evm_version = "shanghai"` per Taiko L2 requirements. `forge-std` is pinned via `contracts/foundry.lock` instead of a checked-in submodule.

**Remaining (non-blocking for downstream plans):**
- **Task 11 cleanup:** run `forge coverage --report summary` and confirm thresholds (≥95% lines, 100% branches); run `slither impl/Heed.sol` and document any medium findings; merge `.github/workflows/contracts.yml` from the stranded `dantaik/heed-contract-only` branch onto `main`.

**Downstream:** the deployed address unblocks both [Plan 2 `heed-core`](./2026-04-28-core-lib.md) (already shipped against this address) and [Plan 4 `heed-web`](./2026-04-28-web.md) (queued for a future PR).
