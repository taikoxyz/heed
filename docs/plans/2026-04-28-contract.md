# Postage.sol Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship the immutable `Postage.sol` contract on Taiko mainnet — fully tested, audited, and verified on Taikoscan.
...
**Spec source:** `docs/postage-design.md`
...
- Create: `contracts/src/interfaces/IPostage.sol`
- Create: `contracts/src/Postage.sol`
- Create: `contracts/test/Postage.t.sol`
- Create: `contracts/test/Postage.invariant.t.sol`
...
```bash
git add contracts/
git commit -m "Bootstrap Foundry workspace for Postage contract"
```

---

### Task 2: Define `IPostage` interface (types + events + signatures)
...
```bash
git add contracts/src/interfaces/IPostage.sol
git commit -m "Add IPostage interface (events, structs, errors)"
```
...
import {IPostage} from "src/interfaces/IPostage.sol";
...
contract PostageTest is Test {
    ...
    function test_publishKey_storesAndEmits() public {
        ...
        emit IPostage.KeyPublished(alice, 0, pub0);
        vm.prank(alice);
        tm.publishKey(0, pub0);

        IPostage.EncKey[2] memory keys = tm.getKeys(alice);
...
Expected: FAIL — `Postage` not found.
...
import {IPostage} from "./interfaces/IPostage.sol";
...
function test_publishKey_rejectsNonMonotonic() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(IPostage.KeyNonceNotMonotonic.selector, 3, 5));
...
Run: `forge test --match-contract PostageTest -vv`
Expected: 3 PASS.

- [ ] **Step 7: Commit**

```bash
git add contracts/src/Postage.sol contracts/test/Postage.t.sol
...
**Files:**
- Modify: `contracts/src/Postage.sol`
- Modify: `contracts/test/Postage.t.sol`
...
function test_setFee_storesAndEmits() public {
    vm.expectEmit(true, false, false, true, address(tm));
    emit IPostage.FeeUpdated(alice, 12345);
    vm.prank(alice);
    tm.setFee(12345);
...
function test_setFee_revertsAboveCap() public {
    vm.prank(alice);
    vm.expectRevert(abi.encodeWithSelector(IPostage.FeeAboveCap.selector, 10_000_001, 10_000_000));
...
function test_trust_addsAndEmits() public {
    ...
    emit IPostage.Trusted(alice, bob, true);
    vm.expectEmit(true, true, false, true, address(tm));
    emit IPostage.Trusted(alice, carol, true);
    vm.prank(alice);
    tm.trust(list);
...
function test_untrust_removesAndEmits() public {
    ...
    tm.trust(list);
    vm.expectEmit(true, true, false, true, address(tm));
    emit IPostage.Trusted(alice, bob, false);
    tm.untrust(list);
    vm.stopPrank();
...
function test_registerDelegate_setsMappingsAndForwardsValue() public {
    ...
    bytes32 clientId = keccak256("postage-mac-v1");
    vm.deal(alice, 1 ether);

    vm.expectEmit(true, true, false, true, address(tm));
    emit IPostage.DelegateRegistered(alice, delegate, clientId);
    vm.prank(alice);
...
function test_revokeDelegate_byOwner() public {
    ...
    vm.expectEmit(true, true, false, true, address(tm));
    emit IPostage.DelegateRevoked(alice, delegate);
...
function test_sendBatch_atomic_singleSuccess() public {
    ...
    IPostage.MailIntent[] memory mails = new IPostage.MailIntent[](1);
    mails[0] = IPostage.MailIntent({
        recipient: bob,
        valueGwei: 100,
    ...
    vm.expectEmit(true, true, false, true, address(tm));
    emit IPostage.MailSent(alice, bob, bytes32(uint256(0xabcd)), 100);
    vm.prank(alice);
...
function test_sendBatch_atomic_revertsOnInsufficientValue() public {
    ...
    IPostage.MailIntent[] memory mails = new IPostage.MailIntent[](1);
    mails[0] = IPostage.MailIntent({recipient: bob, valueGwei: 50, contentRef: bytes32(0)});
...
function test_sendBatch_delegateAttribution() public {
    ...
    IPostage.MailIntent[] memory mails = new IPostage.MailIntent[](1);
    mails[0] = IPostage.MailIntent({recipient: bob, valueGwei: 0, contentRef: bytes32(uint256(1))});

    vm.expectEmit(true, true, false, true, address(tm));
    emit IPostage.MailSent(alice, bob, bytes32(uint256(1)), 0); // sender == owner, not delegate
    vm.prank(delegate);
...
function test_sendBatch_bestEffort_skipsAndRefunds() public {
    ...
    IPostage.MailIntent[] memory mails = new IPostage.MailIntent[](3);
    mails[0] = IPostage.MailIntent({recipient: bob, valueGwei: 100, contentRef: bytes32(uint256(1))});
    mails[1] = IPostage.MailIntent({recipient: address(reverter), valueGwei: 0, contentRef: bytes32(uint256(2))});
    mails[2] = IPostage.MailIntent({recipient: carol, valueGwei: 50, contentRef: bytes32(uint256(3))});
...
function test_sendBatch_atomic_revertsOnReverter() public {
    Reverter reverter = new Reverter();
    IPostage.MailIntent[] memory mails = new IPostage.MailIntent[](1);
    mails[0] = IPostage.MailIntent({recipient: address(reverter), valueGwei: 100, contentRef: bytes32(0)});
    vm.deal(alice, 1 ether);
    vm.prank(alice);
    vm.expectRevert(abi.encodeWithSelector(IPostage.MailFailed.selector, 0));
    tm.sendBatch{value: 100 gwei}(mails, true);
}
```
...
function test_getInbox_returnsFeeAndKeys() public {
    ...
    IPostage.InboxView memory v = tm.getInbox(alice);
...
function test_getInboxes_batched() public {
    ...
    IPostage.InboxView[] memory vs = tm.getInboxes(list);
...
contract Handler is Test {
    Postage public tm;
    constructor(Postage _tm) { tm = _tm; }
    ...
    function send(uint8 nMails, uint96 totalValue, bool atomic, address[3] memory rcpts) external {
        ...
        IPostage.MailIntent[] memory mails = new IPostage.MailIntent[](nMails);
        for (uint256 i; i < nMails; ++i) {
            mails[i] = IPostage.MailIntent({
                recipient: rcpts[i],
...
contract PostageInvariantTest is Test {
    Postage tm;
    Handler handler;
...
Run: `forge test --match-contract PostageInvariantTest -vv`
Expected: PASS — contract never accumulates ETH because it forwards/refunds in the same call.

- [ ] **Step 3: Commit**

```bash
git commit -am "Add invariant: Postage contract holds no ETH after any sendBatch"
```
...
Expected: lines ≥ 95%, branches = 100% on `Postage.sol`. Iterate tests until thresholds hit.
...
Run: `slither contracts/src/Postage.sol --config contracts/slither.config.json`
...
contract Deploy is Script {
    function run() external returns (Postage tm) {
        vm.startBroadcast();
        tm = new Postage(10_000_000);
        vm.stopBroadcast();
    }
...
git commit -m "Record Postage mainnet deployment"
```
