...
import { runInboxList, watchInbox, type InboxMessage } from "../lib/inbox";
...
export function registerInboxCommand(program: Command): void {
  program
    .command("inbox")
    .description(
      "List the agent's inbox: encrypted mail decrypted, decoded, and signature-verified",
    )
    .option(
      "--since-block <n>",
      "Only show messages mined at or after this block number",
    )
    .option(
      "--limit <n>",
      "Cap the number of historical messages (default 50)",
      "50",
    )
    .option("--json", "Print machine-readable JSON instead of compact text")
    .option(
      "--watch",
      "After the historical list, keep listening for new messages until interrupted",
    )
    ...
    .action(async (opts: InboxCliOpts) => {
      try {
        ...
        const deps = {
          ...
          listInbox: (sinceBlock?: bigint, limit?: number) =>
            source.listInbox(walletAddress, sinceBlock, limit),
          watchInbox: (handler: Parameters<typeof source.subscribe>[1]) =>
            source.subscribe(walletAddress, handler),
          fetchByContentRef: (contentRef: `0x${string}`) =>
            fetchByContentRef({ gateway, contentRef }),
        };

        const messages = await runInboxList(
          {
            ...(opts.sinceBlock !== undefined && {
              sinceBlock: BigInt(opts.sinceBlock),
            }),
        ...
        if (opts.watch) {
          const stop = watchInbox(deps, (msg) =>
            renderMessage(msg, opts.json === true),
          );
...
function formatText(msg: InboxMessage): string {
  const ts = new Date(msg.blockTimestamp * 1000)
    .toISOString()
    .replace("T", " ")
    .slice(0, 19);
  ...
  if (msg.decoded.kind === "envelope") {
    ...
    const reply = env.reply_to
      ? ` (reply to ${env.reply_to.slice(0, 10)}…)`
      : "";
...
function serializeForJson(msg: InboxMessage): unknown {
  ...
  return {
    ...
    decoded,
    ...(msg.signatureValid !== undefined && {
      signatureValid: msg.signatureValid,
    }),
    ...(msg.signerMatchesSender !== undefined && {
      signerMatchesSender: msg.signerMatchesSender,
    }),
