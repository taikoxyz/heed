...
export async function runSetup(
  ...
  if (existing && !args.force && !args.importPrivateKey) {
    throw new CliError(
      "BAD_INPUT",
      "a wallet is already configured. pass --force to overwrite, or --import-private-key to replace it",
    );
  }

  const privateKey =
    args.importPrivateKey ??
    (existing && !args.force ? existing : deps.generate());
  ...
  if (!args.noPublish) {
    ...
    if (!rpcUrl) {
      throw new CliError(
        "RPC_NOT_CONFIGURED",
        "no RPC URL configured. pass --rpc-url, set network.rpc_url via `heed config set`, or use --no-publish",
      );
    }
  ...
  return {
    ...
    encryptionPub: keys.encryptionPub,
    keyNonce,
    txHash,
    imported,
  };
...
export function registerSetupCommand(program: Command): void {
  program
    .command("setup")
    .description(
      "Generate or import a wallet, derive the encryption key, and publish it on-chain",
    )
    .option(
      "--import-private-key <hex>",
      "Import an existing 0x-prefixed 64-char private key",
    )
    .option(
      "--rpc-url <url>",
      "Override network.rpc_url for the publishKey transaction",
    )
    .option(
      "--no-publish",
      "Generate keys without sending the publishKey transaction (useful offline)",
    )
      ...
      async (opts: {
        ...
        try {
          const args: SetupArgs = {
            ...(opts.importPrivateKey !== undefined && {
