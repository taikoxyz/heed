...
import { pin, type Urgency } from "@heed/core";
...
export function registerSendCommand(program: Command): void {
  program
    ...
    .description("Send a signed envelope to a recipient wallet, paying their on-chain fee")
    ...
    .option("--reply-to <hash>", "32-byte content reference of the message being replied to")
    .option("--rpc-url <url>", "Override network.rpc_url")
    .option("--dry-run", "Print the signed envelope and synthetic CID without pinning to IPFS or sending on-chain")
    .option("--keystore <kind>", "Keystore source: auto, file, or env", "auto")
    .action(async (recipient: string, opts: SendCliOpts) => {
      try {
        if (!/^0x[0-9a-fA-F]{40}$/.test(recipient)) throw new Error("recipient must be a 0x-prefixed 40-char address");
        if (!URGENCY_VALUES.includes(opts.urgency)) throw new Error(`urgency must be one of: ${URGENCY_VALUES.join(", ")}`);
        if (opts.body !== undefined && opts.bodyFromStdin) throw new Error("--body and --body-from-stdin are mutually exclusive");
        ...
        const lookup = (addr: Address) =>
          lookupRecipient({ rpcUrl, chainId: config.network.chain_id, contract: config.network.contract, recipient: addr });

        if (opts.dryRun) {
          const result = await runSendDryRun(args, { privateKey, config, lookupRecipient: lookup, now: () => Date.now() });
          console.log(JSON.stringify(result, null, 2));
          return;
        }

        const pinataJwt = process.env.HEED_PINATA_JWT;
        if (!pinataJwt) throw new Error("HEED_PINATA_JWT must be set to pin encrypted bytes to IPFS.");

        const result = await runSend(args, {
          privateKey,
          config,
          lookupRecipient: lookup,
          pin: (bytes, name) => pin(bytes, name, { jwt: pinataJwt }),
