...
function fakeClient(logs: FakeLog[], opts: { tip?: bigint } = {}) {
  const tip =
    opts.tip ??
    logs.reduce((m, l) => (l.blockNumber > m ? l.blockNumber : m), 0n);
  return {
      ...
      async ({ fromBlock, toBlock }: { fromBlock: bigint; toBlock?: bigint }) =>
        logs.filter(
          (l) =>
            ...
            (toBlock === undefined || l.blockNumber <= toBlock),
        ),
    ),
    getBlock: vi.fn(async ({ blockNumber }: { blockNumber: bigint }) =>
      fakeBlock(blockNumber, blockNumber * 10n),
    ),
    watchEvent: vi.fn(),
    readContract: vi.fn(),
  } as any;
...
describe("createRpcMailSource", () => {
  it("returns an object satisfying the MailSource interface", () => {
    const src = createRpcMailSource({
      client: fakeClient([]),
      contract: DEAD_CONTRACT,
      deployedAtBlock: 0n,
    });
  ...
  it("maps logs to MailEvent[] for listOutbox", async () => {
    const client = fakeClient([
      log(2, 0, { sender: "0xAlice", recipient: "0xBob", valueGwei: 10n }),
    ]);
  ...
  it("scans backward in bounded windows down to the floor when the range is empty", async () => {
    ...
    expect(client.getLogs).toHaveBeenLastCalledWith(
      expect.objectContaining({ fromBlock: 0n, toBlock: 0n }),
    );
  });
